<template>
  <div class="animate-fade-up">
    ...
    <!-- ── Metadata header ── -->
    <div class="glass rounded-2xl border border-white/8 p-5 mb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">

      <!-- Left: meta chips -->
      <div class="flex flex-wrap gap-4">
        <div v-for="meta in metaItems" :key="meta.label" class="flex flex-col gap-0.5">
          <span class="text-[10px] font-mono uppercase tracking-widest text-gray-600">{{ meta.label }}</span>
          <span class="text-sm font-semibold text-white">{{ meta.value }}</span>
        </div>
      </div>
      ...
      <div class="flex gap-2 shrink-0">
        ...
        <button
          ...
          class="btn-amber flex items-center gap-2 px-4 py-2 text-sm"
        >
          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <!-- ── Report prose ── -->
    <div class="glass rounded-2xl border border-white/8 p-8 md:p-12 shadow-2xl">
      <div
...
const metaItems = computed(() => [
  ...
  { label: "Sources", value: store.citations.length > 0 ? `${store.citations.length} sources` : "—" },
  { label: "Time",    value: store.elapsedSeconds ? `${store.elapsedSeconds}s` : "—" },
  { label: "Date",    value: store.generatedAt ? new Date(store.generatedAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }) : "—" },
]);
